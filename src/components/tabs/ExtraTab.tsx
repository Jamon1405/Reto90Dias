  ...
  return (
    <div className="space-y-4">
      <div className="grid gap-3 sm:grid-cols-4">
        ...
        <div>
          <label className="text-xs font-semibold">Factor MET</label>
          <Input
            type="number"
            value={form.factor}
      ...
      <div className="space-y-2">
        {data.extraBurnJson.events.length === 0 ? (
          <p className="text-xs text-slate-500">Sin eventos extra.</p>
        ) : (
          data.extraBurnJson.events.map((event, index) => (
            <div key={`${event.type}-${index}`} className="flex items-center justify-between rounded-md border border-slate-200 px-3 py-2 text-xs">
              <div>
                <div className="font-semibold">{event.type}</div>
                <div>{event.min} min · MET {event.factor} · {event.cals} kcal</div>
                {event.notes && <div className="text-slate-500">{event.notes}</div>}
              </div>
